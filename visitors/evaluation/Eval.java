...
public class Eval implements Visitor<Value> {
	...
	public Value visitForStmt(VarIdent ident, Exp exp, Block block) {
		ident.accept(this).toInt();
		exp.accept(this).toInt();
		while (ident.accept(this).compare(exp.accept(this)) || ident.accept(this).equals(exp.accept(this))) { //While index <= max executes loop
			block.accept(this);
			Value payload = new IntValue(env.lookup(ident).toInt()+1); //index value raised by 1 each cycle
			env.update(ident, payload);
		}
		return null;
	}
	...
	public Value visitSeasonOf(Exp exp) { 
		return new SeasonValue(exp.accept(this).intToSeason()); 
	}
